...
def create_account(token):
    ...
    if not tok:
        return render_template("token_invalid.html", reason="This link is invalid.")
    if tok["used_at"]:
        return render_template("token_invalid.html", reason="This link has already been used.")
    if datetime.now() > tok["expires_at"]:
        return render_template("token_invalid.html", reason="This link has expired.")
    ...
    if request.method == "POST":
        ...
        if not errors:
            ...
            if cur.fetchone():
                ...
                cur.execute(
                    "INSERT INTO users (role, fname, lname, email, password_hash, contact_number, company_name, payment_status) "
    ...
    return render_template("create_account.html", tok=tok, errors=errors)
